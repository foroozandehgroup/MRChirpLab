...
pulse_param.Q = 2*param.Q180;
p2 = MRchirp(pulse_param);

% pulse 3: pi pulse
pulse_param.tp = tau(3);
pulse_param.delta_t = tau(1) + tau(2) + tau(3)/2;
pulse_param.Q = param.Q180;
...
seq.tau = tau;
seq.pulses = {p1, p2, p3};
seq.total_time = p3.delta_t + p3.tp / 2;

% 64 steps phase cycling for 6 pi pulses
ph1 = repmat([0 1 2 3],1,16);
ph2 = repmat(repelem([0 1 2 3],4),1,4);
ph3 = repelem([0 1 2 3],16);

CTP = [+2 -2 +2]; % coherence transfer pathway
...
if isfield(param, 't_delay')
    ...
    seq = seq_add_delay(seq, t_delay/2, 4);
    seq = seq_add_delay(seq, t_delay/2, 1);
    
%     % delay in beetween pulses
%     seq = seq_add_delay(seq, t_delay/4, 4);
%     seq = seq_add_delay(seq, t_delay/4, 3);
% 	  seq = seq_add_delay(seq, t_delay/4, 2);
%     seq = seq_add_delay(seq, t_delay/4, 1);
